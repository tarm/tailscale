...
func runTests(ctx context.Context, attempt int, pt *packageTests, otherArgs []string, ch chan<- *testAttempt, f *os.File) {
	defer close(ch)
...
func main() {
	...
	for len(toRun) > 0 {
		...
		for _, pt := range thisRun.tests {
			...
			go runTests(ctx, thisRun.attempt, pt, otherArgs, ch, f)
			for tr := range ch {
				if tr.pkgFinished {
